	...
	export class NumberTest extends Unit.Fixture {
		constructor() {
			...
			this.add("integer #1", () => {
				...
				this.expect((token = Number.scan(source)) instanceof Number, Is.True())
				this.expect((<Number>token).getValue(), Is.Equal().To(parseInt(numberString)))
			})
			this.add("integer #2", () => {
			...
			this.add("float #1", () => {
				...
				var source = new Source(new IO.StringReader(numberString), errorHandler)
				this.expect((token = Number.scan(source)) instanceof Number, Is.True())
				this.expect((<Number>token).getValue(), Is.Equal().To(parseFloat(numberString)))
			})
			this.add("float #2", () => {
			...
			this.add("binary #1", () => {
				...
				this.expect((token = Number.scan(source)) instanceof Number, Is.True())
				this.expect((<Number>token).getValue(), Is.Equal().To(0))
			})
			this.add("binary #2", () => {
				var binaryString = "0b100000001"
				var source = new Source(new IO.StringReader(binaryString), errorHandler)
				this.expect((token = Number.scan(source)) instanceof Number, Is.True())
				this.expect((<Number>token).getValue(), Is.Equal().To(257))
			})
			this.add("octal #1", () => {
				var octalString = "0c0"
			...
			this.add("octal #2", () => {
				...
				var source = new Source(new IO.StringReader(octalString), errorHandler)
				this.expect((token = Number.scan(source)) instanceof Number, Is.True())
				this.expect((<Number>token).getValue(), Is.Equal().To(8))
			})
			this.add("octal #3", () => {
				var octalString = "0c20000"
				var source = new Source(new IO.StringReader(octalString), errorHandler)
				this.expect((token = Number.scan(source)) instanceof Number, Is.True())
				this.expect((<Number>token).getValue(), Is.Equal().To(8192))
			})
			...
			this.add("hexadecimal #2", () => {
				...
				var source = new Source(new IO.StringReader(hexadecimalString), errorHandler)
				this.expect((token = Number.scan(source)) instanceof Number, Is.True())
				this.expect((<Number>token).getValue(), Is.Equal().To(parseInt(hexadecimalString, 16)))
			})
